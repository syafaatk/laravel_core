				...
				<div class="card-body RealPages" style="display: none;">
					<div class="float-right mt-n2" id="PenjualanRetail">
						...
						<a href="javascript:void(0)" class="btn btn-sm btn-info" data-toggle="modal" data-target=".form-addBrand">
							<i class="mdi mdi-plus-box-outline"></i> Tambah Brand / Merk Baru
						</a>
						@endif
					...
					<h4 class="mt-0 header-title">Semua Data Brand / Merk<hr></h4>
					<p class="text-muted font-14 mb-2"></p>
					<ul class="nav nav-tabs">
						<li class="nav-item">
							@if (!empty($Role))
							<a href="#master_brand" data-toggle="tab" aria-expanded="false" class="nav-link active">
								<span> > Data Brand / Merk</span>            
							</a>
							@endif
						</li>
					</ul>
							...
							<form class="col-12 form-inline text-left">
								<div class="col-md-4 mb-1 form-group form-group-custom">
									<select class="form-control select2" id="statusBrand">
										<option value="All">Semua Status Status Brand / Merk</option>
										<option value="Active">Status Aktif</option>
										<option value="Non Active">Status Non Aktif</option>
									</select>
								</div>
								<div class="col-md-2">
									<button type="button" class="btn btn-sm btn-success btn-block ReloadDataSelling"><i class="mdi mdi-calendar-search"></i> Filter Brand / Merk</button>
								</div>
							</form>
						</div>
						@endif
						@if (!empty($Role))
						<div role="tabpanel" class="tab-pane fade show active" id="master_brand">
...
@include('master.sales.brand.update')
@endif
...
<script type="text/javascript">
	$(document).ready(() => {
		...
		var columns = [
		{ data : "master_brand_id", name : "master_brand_id", title : "No", render: function ( data, type, row, meta ) {
			return meta.row+meta.settings._iDisplayStart+1;
		}},
		{ data : "master_brand_alias", name : "master_brand_alias", title : "Inisial/Singkatan"},
		{ data : "master_brand_name", name : "master_brand_name", title : "Nama Brand / Merk",},
		{ data : "master_brand_status", name : "master_brand_status", title : "Status Brand / Merk", render: function ( data, type, row, meta ) {
		...
		{ data : "master_brand_id", name : "master_brand_id", title : "", render: function ( data, type, row, meta ) {
			let STATUS = '';
			@if (isValidRole($Role,'Active Nonactive'))
			if (row.master_brand_status=="Active") {
		...
		var datatable_brand = $('#datatable_brand').DataTable({
			...
			"pageLength" : 25,
			ajax : {
				url: '{{ route('master.sales.brand.datatable') }}',
				type: 'POST',
				data: (params) => {
					params._token = $('meta[name="csrf-token"]').attr('content');
					params.master_brand_status = $('#statusBrand').val();
				}
			},
			"aoColumns" : columns,
			...
			$(".ActBrand").on('click', function(event) {
				...
				Swal.fire({
					...
					if (result.value) {
						...
						@if (isValidRole($Role,'Active Nonactive'))
						if (action == "Active") {
							$.getJSON(`{{ route('master.sales.brand.active', ['id' => '']) }}/${id}`, function(data) {
								msg = data.messages;
							}).then(()=>{ Swal.fire(action, msg ,'success'); });
						}

						if (action == "Non Active") {
							$.getJSON(`{{ route('master.sales.brand.nonactive', ['id' => '']) }}/${id}`, function(data) {
								msg = data.messages;
							}).then(()=>{ Swal.fire(action, msg ,'success'); });
						}
						@endif
						...
						if (action == "Penghapusan Data") {
							$.post(`{{ route('master.sales.brand.delete', ['id' => '']) }}/${id}`,{_token:$('meta[name="csrf-token"]').attr('content')}, function(data) {
								msg = data.messages;
							}).then(()=>{ Swal.fire(action, msg ,'success'); });
						}
						@endif
