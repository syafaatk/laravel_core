  ...
  <div class="modal fade form-updateWarehouse" role="dialog" aria-labelledby="addWarehouse" aria-hidden="true" style="display: none;">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title" id="addWarehouse">Update Gudang</h4>
          <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
        </div>
        <div class="modal-body">
          <form action="{{ $route->update }}" method="POST" class="form-horizontal ModalAjax onUpdateWarehouse" accept-charset="utf-8">
            @csrf
            <div class="form-group row">
              <label class="col-md-3 mb-n3 col-form-label">Inisial/Singkatan Gudang</label>
              <div class="col-md-9 mb-n3">
                <input type="text" class="form-control form-control-sm" maxlength="6" id="update-master_warehouse_alias" name="master_warehouse_alias" maxlength="6" placeholder="Inisial/Singkatan Gudang (Max 6 Karakter)" required="">
              </div>
            </div>
            <div class="form-group row">
              <label class="col-md-3 mb-n3 col-form-label">Nama Gudang</label>
              <div class="col-md-9 mb-n3">
                <input type="text" class="form-control form-control-sm" id="update-master_warehouse_name" name="master_warehouse_name" placeholder="Nama Gudang" required="">
              </div>
            </div>
            <div class="form-group row">
  ...
  <script type="text/javascript">
    function updateWarehouse(){
      $(".ActupdateWarehouse").unbind();
      $('.ActupdateWarehouse').on('click', function(event) {
        event.preventDefault();

        let id = $(this).data('unique');
        let msg = "";

        $('.onUpdateWarehouse').attr('action',`{{ $route->update }}/${id}`);
